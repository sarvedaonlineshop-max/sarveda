/**
 * Phase 4E Lightsail final banking close-out validation.
 *
 * Extends Phase 4D E2E with money-parse regression, reserved-GL guard,
 * commerce count fingerprints, and fixture register dump.
 * Process-scoped flags only — leave Lightsail .env OFF.
 *
 *   PHASE4E_LIGHTSAIL_FINAL_OK=1 \
 ...
 *   npx tsx scripts/phase4e-lightsail-final-validation.ts
 */
...
import { createBankAccount, computeBookBalanceForGlCode } from "../src/modules/accounting/bank-account.service";
...
import { BankReconciliationLockedError, BankAccountInvalidError } from "../src/modules/accounting/accounting-errors";
import { unmatchStatementLine } from "../src/modules/accounting/bank-statement-matching.service";

const prisma = new PrismaClient();
const TAG = `TEST-ACC-BANK-4E-${Date.now()}`;
...
async function main() {
  if (!process.env.PHASE4E_LIGHTSAIL_FINAL_OK) {
    console.error("Set PHASE4E_LIGHTSAIL_FINAL_OK=1 to run on Lightsail");
    process.exit(1);
  }
  ...
  console.log(`\n=== Phase 4E Lightsail Final — ${TAG} ===\n`);

  await seedAccountingChartOfAccounts();

  const commerceCountsBefore = {
  ...
  console.log("COMMERCE_COUNTS_BEFORE", JSON.stringify(commerceCountsBefore));

  // Critical money parse regression via statement commit
  const moneyProbeGl = `32${String(Date.now() % 100).padStart(2, "0")}`;
  const moneyBank = await createBankAccount({
    name: `${TAG}-MONEY`,
    bankName: "TEST",
    glAccountCode: moneyProbeGl,
    accountType: "BANK",
    createGlIfMissing: true,
    statementImportEnabled: true
  });
  const moneyImp = await commitBankStatementImport({
    bankAccountId: moneyBank.id,
    fileName: `${TAG}-money.csv`,
    buffer: csvBuffer([["2026-08-20", "BANK CHARGES", "", "590.00", "", ""]])
  });
  ok("A0 590.00 → 59000 paise (never 5900000)", moneyImp.lines[0]!.debitInPaise === 59_000, String(moneyImp.lines[0]!.debitInPaise));
  ...
  try {
    ...
    ok("A1 reserved GL 1020 blocked", false);
  } catch (e) {
    ok(
      "A1 reserved GL 1020 blocked",
      e instanceof BankAccountInvalidError && e.code === "GL_RESERVED_NON_BANK"
    );
  }

  const suffix = String(Date.now() % 100).padStart(2, "0");
  const glHdfc = `33${suffix}`;
  const glIcici = `34${suffix}`;
  ...
  // Opening ₹10,00,000 (10 lakh = 1_000_000 paise in prior scripts; here 1_000_000 paise = ₹10,000)
  ...
  ok(
    "A2 book balance after opening",
    (await computeBookBalanceForGlCode(hdfc.glAccountCode)) === 1_000_000
  );
  ...
  console.log("COMMERCE_COUNTS_AFTER", JSON.stringify(commerceCountsAfter));
  ok(
    "V orders/payments/refunds/inventory/PO unchanged",
    commerceCountsBefore.orders === commerceCountsAfter.orders &&
      commerceCountsBefore.payments === commerceCountsAfter.payments &&
      commerceCountsBefore.refunds === commerceCountsAfter.refunds &&
      commerceCountsBefore.inventory === commerceCountsAfter.inventory &&
      commerceCountsBefore.purchaseOrders === commerceCountsAfter.purchaseOrders
  );
  ok(
    "W vendorBills delta is exactly +1 TEST bill (expected fixture)",
    commerceCountsAfter.vendorBills === commerceCountsBefore.vendorBills + 1,
    `before=${commerceCountsBefore.vendorBills} after=${commerceCountsAfter.vendorBills}`
  );
  ...
  const flagLines = envText
    .split("\n")
    .filter((l) =>
      /^(NATIVE_ACCOUNTING_ENABLED|ACCOUNTING_BANKING_ENABLED|ACCOUNTING_BANK_STATEMENT_IMPORT_ENABLED|ACCOUNTING_BANK_RECONCILIATION_ENABLED|ACCOUNTING_PRODUCTION_POSTING_ALLOWED|ACCOUNTING_COD_COLLECTION_ENABLED)=/i.test(
        l.trim()
      )
    );
  ok(
  ...
  console.log("\n=== PRE-PRODUCTION FIXTURE REGISTER (this run) ===");
  console.log(
    JSON.stringify(
      {
        tag: TAG,
        hdfcBankAccountId: hdfc.id,
        hdfcGl: hdfc.glAccountCode,
        iciciBankAccountId: icici.id,
        iciciGl: icici.glAccountCode,
        moneyProbeBankAccountId: moneyBank.id,
        moneyProbeGl: moneyBank.glAccountCode,
        reconciliationId: closed.id,
        statementImportId: imp.id,
        transferId: xfer.id,
        vendorBillId: bill.id,
        vendorPaymentUtr: vpUtr,
        razorpaySettlementUtr: rzpUtr
      },
      null,
      2
    )
  ...
  if (results.every(Boolean)) {
    console.log("\nPHASE 4E BANKING FINAL HARDENING VALIDATED");
  } else {
    console.error("\nPHASE 4E VALIDATION FAILED — one or more checks failed");
    process.exit(1);
  }
}
...
main()
  .catch((e) => {
    console.error("PHASE 4E VALIDATION FAILED —", e instanceof Error ? e.message : e);
    process.exit(1);
  })
  .finally(async () => {
