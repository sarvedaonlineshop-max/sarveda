...
export function buildWooCommerceAnalytics(query: AnalyticsQuery) {
  ...
  const overview = {
    kpis,
    orderTrend,
    tips: buildTips(kpis, mostSold[0], dropCandidates[0], refundsInRange.length)
  };
...
function buildTips(
  ...
  if (topProduct) {
    tips.push(
      `${topProduct.productName} is your top seller in this range (${topProduct.unitsSold} units) — protect stock and feature it.`
    );
  }
  if (kpis.uniqueCustomers > 0) {
    const repeatPct = Math.round((kpis.repeatCustomerCount / kpis.uniqueCustomers) * 100);
    tips.push(`Repeat purchase rate is about ${repeatPct}% in this range — nurture VIPs with post-purchase offers.`);
  }
  if (kpis.aovInr > 0) {
    tips.push(`Average order value is ₹${kpis.aovInr.toLocaleString("en-IN")} — test bundles to lift AOV.`);
  }
  if (refundN > 0) {
    tips.push(`${refundN} refund(s) in range — review reasons and product QA for repeat refund SKUs.`);
  }
  if (drop) {
    tips.push(`${drop.productName} is a slow mover (${drop.unitsSold} units) — consider discount, bundle, or delist.`);
  }
  if (tips.length === 0) tips.push("No completed sales in this date range.");
  return tips;
}
