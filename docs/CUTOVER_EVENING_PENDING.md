...
- [x] **PRODUCTS SOURCE 3** created — native feed URL  
  `https://sarveda-demo.xyz/api/merchant/google/products.xml` · **790** products, fetch OK  
  Feed label currently **IN** (same as Content API — watch for clashes; prefer rename to `NATIVE` if editable)  
  Tonight ~21:30: pause Source 2; switch Source 3 URL → `https://sarveda.com/api/merchant/google/products.xml`
