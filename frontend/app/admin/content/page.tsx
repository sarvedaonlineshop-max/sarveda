    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        {ADMIN_CONTENT_TYPES.map((t) => (
          <button key={t} type="button" onClick={() => setType(t)}
            style={{
              ...
              color: t === type ? "#1c352a" : "#8a7060",
              borderBottom: t === type ? "2px solid #b98a3e" : "2px solid transparent",
              marginBottom: "-2px",
      ...
      <div
        style={{
          ...
          ...card,
          padding: "16px 20px",
          background: "linear-gradient(180deg, #faf9f6, #ffffff)",
      ...
      <div style={{ ...card, overflowX: "auto", boxShadow: "0 4px 20px rgba(28,53,42,0.07)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {items.map((row) => (
              <tr key={row.id} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "#faf5ec"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = ""; }}>
                <td style={{ ...tdSt, fontWeight: 700, color: "#1c352a" }}>{row.title}</td>
                <td style={{ ...tdSt, fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontSize: "12px", color: "#b98a3e" }}>{row.slug}</td>
                ...
                <td style={{ ...tdSt, whiteSpace: "nowrap" }}>
                  <Link
                    ...
                    style={{ fontSize: "13px", fontWeight: 600, color: "#b98a3e", textDecoration: "none", fontFamily: "'Plus Jakarta Sans', sans-serif" }}
                  ...
                  <button type="button" disabled={busyId === row.id} onClick={() => void deactivate(row)}
                    style={{ marginLeft: "16px", fontSize: "13px", color: "#8a7060", background: "none", border: "none", cursor: "pointer", padding: 0, opacity: busyId === row.id ? 0.4 : 1, transition: "color 0.15s" }}
