...
export default function BankTransfersPage() {
  ...
  async function continuePreview() {
    ...
    } catch (e) { setNotice({ tone: "error", text: e instanceof Error ? e.message : "Transfer preview could not be prepared." }); }
  ...
  async function post() {
    ...
    try {
      ...
      setNotice({ tone: "success", text: `Transfer recorded in journal ${result.journal.entryNumber}.` });
  ...
  return (
    <BankingPageShell title="Transfers" subtitle="Move money between bank and cash accounts with a journal preview before posting." actions={<button className={accountingButtonClass()} onClick={() => setShowForm((v) => !v)}>New Transfer</button>}>
      {!enabled ? <FeatureUnavailable>Banking posting is currently unavailable. Transfer history remains available to review.</FeatureUnavailable> : null}
      {notice ? <AccountingAlert tone={notice.tone}>{notice.text}</AccountingAlert> : null}
      {showForm ? <AccountingSectionCard><AccountingSectionHeader title="New Transfer" description="Choose one of the supported money movements." />
        ...
        {!preview ? <button className={`mt-4 ${accountingButtonClass()}`} disabled={busy || !enabled || !form.source || !form.destination || form.source === form.destination || !Number.isFinite(amount) || amount <= 0} onClick={() => void continuePreview()}>Continue to Preview</button> : <div className="mt-5 rounded-xl border border-[#e0d8ce] p-4"><h3 className="font-semibold text-[#1c352a]">Journal preview</h3><p className="mt-1 text-xs text-[#75675e]">{transferKindLabel(form.transferKind)} · {formatBankDate(form.transferDate)} · {formatInrPaise(amount)}</p><div className="mt-3 overflow-x-auto"><table className="min-w-full"><thead><tr><th className={bankingTh()}>Ledger Account</th><th className={bankingTh(true)}>Debit</th><th className={bankingTh(true)}>Credit</th></tr></thead><tbody>{lines.map((line, i) => <tr key={i} className="border-t border-[#eee8e0]"><td className={bankingTd()}>{String(line.lineMemo ?? line.accountCode ?? "Account")} <span className="text-xs text-[#75675e]">{String(line.accountCode ?? "")}</span></td><td className={`${bankingTd(true)} ${moneyClass()}`}>{Number(line.debitInPaise) ? formatInrPaise(Number(line.debitInPaise)) : "—"}</td><td className={`${bankingTd(true)} ${moneyClass()}`}>{Number(line.creditInPaise) ? formatInrPaise(Number(line.creditInPaise)) : "—"}</td></tr>)}</tbody></table></div><div className="mt-4 flex gap-2"><button className={accountingButtonClass()} disabled={!enabled || busy} onClick={() => setConfirm(true)}>Record Transfer</button><button className={accountingButtonClass("secondary")} onClick={() => { setPreview(null); setDraftId(""); }}>Edit</button></div></div>}
      ...
      <AdminConfirmModal open={confirm} title="Record this transfer?" message="This posts the balanced debit and credit shown in the preview. The posted transfer cannot be edited." details={[`${transferKindLabel(form.transferKind)} · ${formatInrPaise(amount)}`]} confirmLabel="Record Transfer" busy={busy} onConfirm={() => void post()} onClose={() => setConfirm(false)} />
    </BankingPageShell>
  );
}
