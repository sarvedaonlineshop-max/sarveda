...
export default function AdminReturnCaseDetailPage() {
  ...
  const currentStep = isCancellation ? cancellationStageIndex(request, isCod) : stageIndex(request);
  ...
  return <div className="mx-auto max-w-[1380px] space-y-5 p-5 lg:p-7">
    ...
    <section className="rounded-[26px] border border-stone-200 bg-white p-6 shadow-[0_8px_28px_rgba(15,23,42,.05)]">
      ...
      {isCancellation && pending ? <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50/60 p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-base font-extrabold text-amber-950">Pre-dispatch cancellation</p><p className="mt-1 text-sm leading-6 text-amber-900/80">{isCod ? "COD order: approval cancels the order and restores stock. No customer refund or reverse pickup is required." : "Prepaid order: this is the pre-delivery shortcut. Approval cancels the order, restores stock, and sends the full captured-payment refund to the original payment method immediately — no pickup or QC flow."}</p></div><div className="flex flex-wrap gap-2"><button disabled={busy!==null} onClick={()=>void run("approve-cancellation",()=>approveServiceRequest(order.id,request.id,note.trim()||undefined))} className="rounded-xl bg-emerald-700 px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{isCod ? "✓ Approve cancellation — no refund" : `✓ Approve & refund ${formatMinorFromPaise(order.grandTotalInPaise, order.currency)}`}</button><button disabled={busy!==null||!note.trim()} onClick={()=>void run("reject-cancellation",()=>rejectServiceRequest(order.id,request.id,note.trim()))} className="rounded-xl border border-red-200 bg-white px-5 py-2.5 text-sm font-bold text-red-700 disabled:opacity-50">✕ Reject request</button></div></div><input value={note} onChange={(e)=>setNote(e.target.value)} className="mt-4 w-full rounded-xl border border-amber-200 bg-white px-4 py-2.5 text-sm" placeholder="Admin note (required for rejection; optional for approval)"/></div> : null}
      {isCancellation && request.status === "APPROVED" ? <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-5"><p className="text-base font-extrabold text-emerald-950">{isCod ? "Cancellation completed — no refund required" : request.refundProcessedAt || request.resolutionStatus === "REFUNDED" ? "Cancellation and refund completed" : "Cancellation approved — refund processing"}</p><p className="mt-1 text-sm leading-6 text-emerald-900/80">{isCod ? "Payment method: COD. The order is cancelled and stock is restored. There was no captured online payment, so this case is complete and no refund action is required." : request.refundProcessedAt || request.resolutionStatus === "REFUNDED" ? `The captured payment${request.refundTotalInPaise ? ` of ${formatMinorFromPaise(request.refundTotalInPaise, order.currency)}` : ""} has been refunded to the original payment method. No reverse pickup or QC is required for this pre-dispatch cancellation.` : "The order is cancelled and stock is restored. The gateway refund has been initiated; this cancellation stays at the Refund step until the refund is recorded as completed."}</p></div> : null}
