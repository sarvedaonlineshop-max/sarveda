      ...
      footer={
        <div className="space-y-3 text-center text-sm text-brand-muted">
          {!adminOnly ? (
            <p>
              New here?{" "}
              <Link href={`/signup${next ? `?next=${encodeURIComponent(next)}` : ""}`} className="text-brand-gold hover:text-brand-gold-bright">
                Create an account
              </Link>
            </p>
          ) : null}
          <Link href="/" className="block text-brand-gold hover:text-brand-gold">
            Back to storefront
          </Link>
          <button
            type="button"
            className="text-brand-muted underline decoration-brand-lavender-mid/50 hover:text-brand-lavender"
            onClick={() => {
