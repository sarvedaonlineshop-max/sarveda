/** Shared course session / curriculum parsing (admin + import). */
...
/** Parse `<h3>Session N: Title</h3> By Teacher …` blocks from migrated HTML. */
export function parseSessionsFromHtml(html: string): CourseSession[] {
  if (!html?.trim()) return [];

  const sessions: CourseSession[] = [];
  const re = /<h3[^>]*>\s*Session\s+(\d+)\s*:\s*([^<]+)<\/h3>/gi;
  const headers: Array<{ index: number; sessionId: string; name: string; end: number }> = [];
  ...
  for (let i = 0; i < headers.length; i++) {
    const h = headers[i]!;
    const nextStart = headers[i + 1]?.index ?? html.length;
    const block = html.slice(h.end, nextStart).trim();
    ...
    sessions.push({
      ...
      name: decodeHtmlEntities(h.name),
      teacherName,
      content,
      scheduledAt: null,
      scheduleNote: null
...
/** Parse session date lines from course_includes schedule block. */
...
export function applySessionSchedules(
  ...
  return sessions.map((s) => {
    const n = parseInt(s.sessionId, 10);
    const note = lines.get(n);
