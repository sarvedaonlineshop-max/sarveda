...
export function AdminOrderReturnReplacementPanel({
  ...
  const canDisposition = rs?.receivedAt && !rs.disposition;
  const canRefund =
    request.status === "APPROVED" &&
    (!needsReturn || (rs?.receivedAt && rs.disposition && rs.disposition !== "NEEDS_REVIEW"));
  ...
  return (
    <div className="mt-4 space-y-4 rounded-xl border border-amber-200/80 bg-amber-50/40 p-4 dark:border-amber-900/50 dark:bg-amber-950/20">
      ...
      {request.items?.map((item) => (
        <div key={item.id} className="rounded-lg border border-stone-200 bg-white p-3 text-sm dark:border-stone-700 dark:bg-stone-900">
          <p className="font-semibold">{item.nameSnapshot}</p>
          <p className="text-xs text-stone-500">
            Qty {item.qtySelected} · {item.reasonLabel}
            {item.requestedResolution ? ` · ${RESOLUTION_LABELS[item.requestedResolution] ?? item.requestedResolution}` : ""}
          </p>
          {item.refundAmountInPaise != null && item.refundAmountInPaise > 0 ? (
            <p className="text-xs text-green-700">Refunded {formatMinorFromPaise(item.refundAmountInPaise, currency)}</p>
          ) : null}
        </div>
      ))}
      ...
      {needsReturn && request.status === "APPROVED" ? (
        <div className="space-y-2 rounded-lg border border-stone-200 bg-white p-3 dark:border-stone-700 dark:bg-stone-900">
          ...
          <button
            ...
            onClick={() =>
              void run("shipment", () =>
                adminUpdateReturnShipment(orderId, request.id, { courier, awb, physicalStatus: "IN_TRANSIT" })
              )
            }
          >
      ...
      {request.replacementFulfillments?.map((f) => (
        <div key={f.id} className="flex items-center gap-2 text-sm">
          <span>Replacement ×{f.qty} — {f.status}</span>
          {f.status === "REPLACEMENT_PENDING" ? (
            <button
              ...
              onClick={() =>
                void run(`ship-${f.id}`, () =>
                  adminShipReplacement(f.id, { awb: awb || `REP-${Date.now()}`, courier: courier || "Manual" })
                )
              }
            >
              Mark shipped
