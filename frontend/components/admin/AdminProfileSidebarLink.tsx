...
export function AdminProfileSidebarLink({ onNavigate }: { onNavigate?: () => void }) {
  const pathname = usePathname();
  const nav = useAdminNavOptional();
  const activePath = nav?.activePath ?? pathname;
  const active = activePath === "/admin/profile" || activePath.startsWith("/admin/profile/");

  return (
    <Link
      href="/admin/profile"
      onClick={() => {
        nav?.beginNavigation("/admin/profile");
        onNavigate?.();
      }}
