...
function refundStatusLabel(status: string | null | undefined): string {
  switch (status) {
    case "NOT_APPLICABLE": return "No refund required (COD)";
    case "PENDING": return "Refund held until RTO receipt/QC";
...
export function AdminOrderRtoWorkflow({ orderId, currency, onUpdated }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState(data);
      if (data.anyReceived && data.refundExecutionEnabled) {
  ...
  const isCod = !state.refundExecutionEnabled || shipment.rtoRefundWorkflowStatus === "NOT_APPLICABLE";
  ...
  return (
    <div className="rounded-xl border border-violet-200 bg-white p-4 shadow-sm">
      ...
      <p className="mt-1 text-sm text-stone-600">
        Customer cancelled after carrier pickup. Refund/closure happens only after warehouse receipt and item condition.
      </p>
      ...
      <ol className="mt-4 space-y-4 text-sm">
        ...
        {isCod ? (
          <li className="rounded-lg border border-emerald-200 bg-emerald-50/50 px-3 py-2">
            ...
            <p className="mt-0.5 text-stone-600">No online payment was captured, so refund amount is ₹0. Close after receipt and condition. Shipping/product values are not refunded to customer.</p>
          </li>
        ) : breakdown ? (
          <li className="rounded-lg border border-emerald-200 bg-emerald-50/50 px-3 py-2">
