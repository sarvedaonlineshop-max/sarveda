...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const hideFloatingSubscribe = onProductPage || isStorePage;
    ...
    <MotionConfig reducedMotion="user">
      ...
      <a
        ...
        className={`fixed z-50 flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition hover:scale-105 ${
          useSlimFooter
            ? "bottom-[calc(5.5rem+env(safe-area-inset-bottom,0px))] md:bottom-[calc(var(--storefront-slim-footer-offset)+0.75rem)]"
      ...
      {useSlimFooter ? <SlimFooter /> : <SiteFooter />}
      <BottomNav />
      </div>
    </MotionConfig>
  );
