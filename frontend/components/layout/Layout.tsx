...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const isCheckoutPage = pathname === "/checkout" || (pathname?.startsWith("/checkout/") ?? false);
  ...
  const hideFloatingSubscribe = onProductPage || isCartPage || isCheckoutPage || isStorePage;
  const hideWhatsAppOnMobile = isStorePage;
    ...
    <MotionConfig reducedMotion="user">
      ...
      <main
        className={`bg-brand-cream ${
          isHomePage ? "pb-0" : "pb-[calc(4.5rem+env(safe-area-inset-bottom,0px))] md:pb-0"
        }`}
      >
      ...
      <a
        ...
        className={`fixed z-50 flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition hover:scale-105 bottom-24 md:bottom-6 ${
          hideWhatsAppOnMobile ? "hidden md:flex" : ""
      ...
      {hideFloatingSubscribe ? null : <FloatingSocialSubscribe />}
      <SiteFooter />
      <BottomNav />
      </div>
    </MotionConfig>
