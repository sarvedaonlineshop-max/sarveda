...
const columnHeadingClass =
  "mb-4 text-[10px] font-normal uppercase tracking-[0.2em] text-brand-lavender";

const footerLinkClass =
  "text-[13px] font-light text-[rgba(196,176,232,0.45)] transition-colors hover:text-brand-lavender";

const dividerBorder = { borderColor: "rgba(196,176,232,0.12)" } as const;
  ...
  return (
    <footer
      ...
      style={{
        ...
        borderTopColor: "rgba(196,176,232,0.12)",
      }}
    >
      {/* Trust Bar */}
      <div className="border-b border-t" style={{ ...dividerBorder, background: "#1A1208" }}>
        <div className="mx-auto grid max-w-7xl grid-cols-4 divide-x divide-[rgba(196,176,232,0.12)] px-4 sm:px-6 lg:px-8">
          {trustBadges.map((b) => (
            <div key={b.text} className="flex items-center justify-center gap-2.5 py-4">
              <span className="text-xl" aria-hidden>
                {b.icon}
              </span>
              <span className="text-sm font-medium text-[rgba(196,176,232,0.6)]">{b.text}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-5 lg:px-8">
        ...
        <div className="lg:col-span-2">
          <Link href="/">
            <p className="display-text text-[28px] italic leading-tight text-brand-lavender">
              ☸ Sarveda
            </p>
            <p className="mt-0.5 text-[10px] uppercase tracking-[0.2em] text-[rgba(196,176,232,0.45)]">
              YOGA · AYURVEDA · SOUND
            </p>
          </Link>
          <p className="mt-5 max-w-sm text-[13px] font-light leading-[1.75] text-[rgba(196,176,232,0.45)]">
          ...
          <div className="mt-6 flex gap-3">
            {[
              {
                label: "Instagram",
                href: "https://instagram.com/sarveda",
              ...
              <a
                ...
                aria-label={s.label}
                className="flex h-9 w-9 items-center justify-center rounded-lg text-[rgba(196,176,232,0.5)] transition-colors hover:bg-[rgba(196,176,232,0.18)]"
                style={{ background: "rgba(196,176,232,0.1)" }}
              >
        ...
        {/* Support */}
        <div>
          <p className={columnHeadingClass}>Support</p>
          <ul className="space-y-2.5">
      ...
      {/* Bottom strip */}
      <div className="border-t" style={dividerBorder}>
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 py-5 sm:flex-row sm:px-6 lg:px-8">
          <p className="text-[11px] text-[rgba(196,176,232,0.35)]">
            © {new Date().getFullYear()} Sarveda. All rights reserved. Made with 🙏 in India.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-2 text-[rgba(196,176,232,0.35)]">
            <span className="text-[11px] tracking-wide">We accept</span>
            {["Razorpay", "UPI", "Visa", "Mastercard", "Stripe", "PayPal"].map((p) => (
              <span
                key={p}
                className="rounded border px-1.5 py-0.5 text-[10px] font-medium text-[rgba(196,176,232,0.45)]"
